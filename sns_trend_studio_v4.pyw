...
"""
============================================================
 SNS 트렌드 콘텐츠 스튜디오 v4.0  (GUI 앱, 100% 무료)
============================================================
 [v4.0 새 기능] 뉴스 선택만으로 AI 던지기(즉석 프롬프트) / 퍼플렉시티 추가
               무료 이미지 검색(Unsplash·Pexels·Pixabay·구글) 키워드 자동 연결
               미리캔버스·캔바 바로가기 / 이미지 재생성 / 사용 기록 보기
 [v3.2 기능] 해외기사 제목·요약 자동 한국어 번역 (체크박스로 켜고 끔)
 [v3.1 기능] 분야 19개로 확장 / 구글뉴스 해외판 추가 / 해외매체 RSS 30종
 [v3.0 기능]
...
AI_ICONS = {"클로드": "🟠", "ChatGPT": "🟢", "제미나이": "🔵", "퍼플렉시티": "🟣"}

# 무료 이미지 검색 사이트 ({q}에 뉴스 키워드 자동 삽입)
STOCK_SITES = {
    "Unsplash": "https://unsplash.com/s/photos/{q}",
    "Pexels": "https://www.pexels.com/search/{q}/",
    "Pixabay": "https://pixabay.com/images/search/{q}/",
    "구글(상업용)": "https://www.google.com/search?tbm=isch&tbs=il:cl&q={q}",
}
DESIGN_SITES = {
...
# 분야별 수집 소스 정의
# g_kr = 구글뉴스 한국판, g_en = 구글뉴스 미국판(영어, 해외기사)
# ("topic", 이름) = 공식 분야 / ("search", 검색어) = 검색 기반 (둘 다 무료)
CATEGORIES = {
    "🎯 채널 맞춤(자동)":   {"g_kr": None, "g_en": None, "naver": None, "rss": None},
    "💪 건강/피트니스":     {"g_kr": ("topic", "HEALTH"), "g_en": ("topic", "HEALTH"), "naver": ["건강 운동", "다이어트"], "rss": "health"},
    "🏋 운동/헬스 전문":    {"g_kr": ("search", "헬스 근력운동"), "g_en": ("search", "strength training fitness workout"), "naver": ["헬스 트레이닝", "근력 운동"], "rss": "health"},
    "🥗 음식/요리":         {"g_kr": ("search", "음식 요리 레시피"), "g_en": ("search", "food recipe cooking"), "naver": ["요리 레시피", "맛집"], "rss": "food"},
    "✈ 여행":              {"g_kr": ("search", "여행"), "g_en": ("search", "travel destination"), "naver": ["여행 추천"], "rss": "travel"},
    "👗 패션/뷰티":         {"g_kr": ("search", "패션 뷰티 트렌드"), "g_en": ("search", "fashion beauty trend"), "naver": ["패션 트렌드", "뷰티"], "rss": "fashion"},
    "🧠 심리/자기계발":     {"g_kr": ("search", "심리학 습관 자기계발"), "g_en": ("search", "psychology habits self improvement"), "naver": ["심리", "자기계발"], "rss": "lifestyle"},
    "💰 경제/재테크":       {"g_kr": ("topic", "BUSINESS"), "g_en": ("topic", "BUSINESS"), "naver": ["경제", "재테크"], "rss": "business"},
    "🏛 시사/사회":         {"g_kr": ("topic", "NATION"), "g_en": ("topic", "WORLD"), "naver": ["사회 이슈"], "rss": "world"},
    "🤖 IT/테크":           {"g_kr": ("topic", "TECHNOLOGY"), "g_en": ("topic", "TECHNOLOGY"), "naver": ["IT 기술", "AI 인공지능"], "rss": "tech"},
    "🔬 과학":              {"g_kr": ("topic", "SCIENCE"), "g_en": ("topic", "SCIENCE"), "naver": ["과학 연구"], "rss": "science"},
    "🎬 연예/문화":         {"g_kr": ("topic", "ENTERTAINMENT"), "g_en": ("topic", "ENTERTAINMENT"), "naver": ["연예"], "rss": "entertainment"},
    "⚽ 스포츠":            {"g_kr": ("topic", "SPORTS"), "g_en": ("topic", "SPORTS"), "naver": ["스포츠"], "rss": "sports"},
    "🌏 세계/국제":         {"g_kr": ("topic", "WORLD"), "g_en": ("topic", "WORLD"), "naver": ["국제 뉴스"], "rss": "world"},
    "☕ 라이프스타일":      {"g_kr": ("search", "라이프스타일 트렌드"), "g_en": ("search", "lifestyle trend"), "naver": ["라이프스타일 트렌드", "요즘 유행"], "rss": "lifestyle"},
    "🐶 반려동물":          {"g_kr": ("search", "반려동물 강아지 고양이"), "g_en": ("search", "pets dogs cats"), "naver": ["반려동물"], "rss": "pets"},
    "👶 육아/교육":         {"g_kr": ("search", "육아 아이 교육"), "g_en": ("search", "parenting children education"), "naver": ["육아", "교육"], "rss": "lifestyle"},
    "🎮 게임":              {"g_kr": ("search", "게임 신작"), "g_en": ("search", "video games gaming"), "naver": ["게임"], "rss": "tech"},
    "🏠 부동산/인테리어":   {"g_kr": ("search", "부동산 인테리어"), "g_en": ("search", "real estate interior design"), "naver": ["부동산", "인테리어"], "rss": "lifestyle"},
    "🚗 자동차":            {"g_kr": ("search", "자동차 신차"), "g_en": ("search", "cars automotive"), "naver": ["자동차"], "rss": "tech"},
}
...
DEFAULT_RSS = {
    ...
    "food": [
        {"name": "Guardian Food", "url": "https://www.theguardian.com/food/rss"},
        {"name": "NYT Food", "url": "https://rss.nytimes.com/services/xml/rss/nyt/DiningandWine.xml"}
    ],
    "travel": [
        {"name": "Guardian Travel", "url": "https://www.theguardian.com/travel/rss"},
    ...
    "pets": [
        {"name": "ScienceDaily Dogs", "url": "https://www.sciencedaily.com/rss/plants_animals/dogs.xml"},
        {"name": "ScienceDaily Cats", "url": "https://www.sciencedaily.com/rss/plants_animals/cats.xml"}
    ]
}
...
def fetch_google_news(mode, value, lang="ko", limit=8):
    """구글 뉴스 RSS (무료, 키 불필요)
    mode: "topic"(공식 분야) 또는 "search"(검색어)
    lang: "ko"=한국판 / "en"=미국판(해외 기사)"""
    if lang == "en":
        params = "hl=en-US&gl=US&ceid=US:en"
    else:
        params = "hl=ko&gl=KR&ceid=KR:ko"
    if mode == "topic":
        url = f"https://news.google.com/rss/headlines/section/topic/{value}?{params}"
    else:
        url = f"https://news.google.com/rss/search?q={urllib.parse.quote(value)}&{params}"
    ...
    for rank, it in enumerate(root.iter("item")):
        ...
        if title:
            items.append({
                "title": title, "summary": "",
                "link": link,
                "source": "구글" if lang == "ko" else "해외",
                "source_detail": press, "lang": lang,
                "rank": rank,
...
def translate_ko(text):
    """구글 번역 무료 웹 엔드포인트 (키 불필요) → 한국어"""
    if not text.strip():
        return text
    url = ("https://translate.googleapis.com/translate_a/single"
           "?client=gtx&sl=auto&tl=ko&dt=t&q=" + urllib.parse.quote(text[:400]))
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
...
[선택된 뉴스]
- 제목: {news['title']}
{f"- 원문 제목(영어): {news['title_en']}" if news.get('title_en') else ""}
- 요약: {news.get('summary','(요약 없음)')}
{f"- 원문 요약(영어): {news['summary_en']}" if news.get('summary_en') else ""}
...
class App:
    def __init__(self, root):
        self.root = root
        root.title("🚀 SNS 트렌드 콘텐츠 스튜디오 v4.0 (무료)")
    ...
    def _build_main_tab(self):
        ...
        ttk.Checkbutton(top, text="🈯 해외 자동번역",
                        variable=self.trans_var).pack(side="left", padx=8)
        ...
        self.btn_regen = ttk.Button(btns, text="🔄 이미지 다시", command=self.regen_image)
        self.btn_regen.pack(side="left", padx=4)
        ttk.Button(btns, text="🌐 원문 보기", command=self.open_article).pack(side="left", padx=2)
        ttk.Button(btns, text="🖼 이미지 폴더",
                   command=lambda: os.startfile(IMG_DIR)).pack(side="left", padx=2)
        ttk.Button(btns, text="🗂 사용 기록", command=self.show_history).pack(side="left", padx=2)

        # AI 던지기 줄 (프롬프트 없으면 선택한 뉴스로 즉시 생성)
        ai_row = ttk.LabelFrame(self.tab_main, text=" 🤝 AI에게 던지기 — 뉴스만 선택돼 있으면 바로 됩니다 (복사+사이트 열림 → Ctrl+V) ")
        ai_row.pack(fill="x", padx=10, pady=(0, 2))
        for name in AI_SITES:
            ttk.Button(ai_row, text=f"{AI_ICONS.get(name,'🤖')} {name}",
                       command=lambda n=name: self.throw_to_ai(n)).pack(side="left", padx=5, pady=4)
        ttk.Button(ai_row, text="📋 복사만", command=self.recopy).pack(side="left", padx=5)

        # 제작 도구 바로가기 줄
        tool_row = ttk.LabelFrame(self.tab_main, text=" 🧰 제작 도구 — 무료 이미지는 선택한 뉴스 키워드로 자동 검색됩니다 ")
        tool_row.pack(fill="x", padx=10, pady=(0, 4))
        for name in STOCK_SITES:
            ttk.Button(tool_row, text=f"📷 {name}",
                       command=lambda n=name: self.open_stock(n)).pack(side="left", padx=4, pady=4)
        ttk.Separator(tool_row, orient="vertical").pack(side="left", fill="y", padx=6, pady=4)
        for name in DESIGN_SITES:
            ttk.Button(tool_row, text=f"🎨 {name}",
                       command=lambda n=name: webbrowser.open(DESIGN_SITES[n])).pack(side="left", padx=4, pady=4)
    ...
    def start_scan(self):
        ...
        threading.Thread(target=self._scan_worker,
                         args=(ch, self.cat_var.get(), self.trans_var.get()),
                         daemon=True).start()

    def _scan_worker(self, ch, cat_name, do_translate):
        try:
            ...
            if cat_name.startswith("🎯"):
                ...
                g_kr = g_kr or default_cat.get("g_kr")
                g_en = g_en or default_cat.get("g_en")
            if g_kr:
                try:
                    got = fetch_google_news(g_kr[0], g_kr[1], lang="ko", limit=n + 2)
                    self.log(f"  🇬 구글뉴스 한국판 → {len(got)}건")
                    all_items += got
                except Exception as e:
                    self.log(f"  ⚠ 구글뉴스(KR) 오류: {e}")

            # 구글 뉴스 미국판 (해외 기사 대량 확보)
            if g_en:
                try:
                    got = fetch_google_news(g_en[0], g_en[1], lang="en", limit=n + 2)
                    self.log(f"  🌏 구글뉴스 해외판 → {len(got)}건")
                    all_items += got
                except Exception as e:
                    self.log(f"  ⚠ 구글뉴스(해외) 오류: {e}")
            ...
            # 해외 기사 자동번역 (인기도 상위 20건까지, 번역 후 적합도 계산이 정확해짐)
            if do_translate:
                en_items = sorted([it for it in filtered if it["lang"] == "en"],
                                  key=lambda x: x["hot"], reverse=True)[:20]
                if en_items:
                    self.log(f"🈯 해외기사 {len(en_items)}건 자동번역 중... (몇 초 걸려요)")
                    done = 0
                    for it in en_items:
                        try:
                            ...
                            if it.get("summary"):
                                it["summary_en"] = it["summary"]
                                it["summary"] = translate_ko(it["summary"][:180])
                            done += 1
                        except Exception:
                            it["title"] = it.get("title_en", it["title"])
                    self.log(f"  ✅ {done}건 번역 완료")

            # 적합도 계산 + 정렬
            for it in filtered:
                it["fit"] = score_fit(it, ch)
            filtered.sort(key=lambda x: (x["fit"], x["hot"]), reverse=True)
            self.news_list = filtered[:30]
    ...
    def _make_worker(self, news, ch):
        try:
            self.last_news, self.last_channel = news, ch
    ...
    def throw_to_ai(self, ai_name):
        # 프롬프트가 없으면 선택한 뉴스로 즉석 생성 (이미지 없이)
        if not self.last_prompt:
            news = self.selected_news()
            if not news:
                messagebox.showinfo("알림", "목록에서 뉴스를 먼저 선택하세요.\n(또는 [✨ 콘텐츠 만들기]로 이미지까지 함께 생성)")
                return
            ch = self.current_channel()
            self.last_prompt = build_ai_prompt(news, ch, self.guidelines, None)
            self.last_news, self.last_channel = news, ch
            self.history["used"].append({
                "title": news["title"], "link": news.get("link", ""),
                "channel": ch["name"],
            ...
            save_json(HISTORY_PATH, self.history)
            self.log("⚡ 즉석 프롬프트 생성 완료 (배경 이미지가 필요하면 [✨ 콘텐츠 만들기]나 [🔄 이미지 다시]를 쓰세요)")
        self.root.clipboard_clear()
        self.root.clipboard_append(self.last_prompt)
        webbrowser.open(AI_SITES[ai_name])
        self.log(f"🚀 {ai_name} 열림 + 프롬프트 복사 완료 → 채팅창에 Ctrl+V 하세요!")

    # ---------------- 무료 이미지 검색 ----------------
    def _img_query(self):
        news = self.selected_news() or getattr(self, "last_news", None)
        if not news:
            return ""
        q = news.get("title_en") or news["title"]
        return q[:60].strip()

    def open_stock(self, site_name):
        q = self._img_query()
        base = STOCK_SITES[site_name]
        if q:
            url = base.format(q=urllib.parse.quote(q))
            self.log(f"📷 {site_name}에서 '{q[:30]}...' 검색")
        else:
            url = base.split("{q}")[0].rsplit("/", 2)[0] if "{q}" in base else base
            self.log(f"📷 {site_name} 열기 (뉴스를 선택하면 자동 검색됩니다)")
        webbrowser.open(url)
    ...
    def regen_image(self):
        ...
        threading.Thread(target=self._regen_worker, args=(news, ch), daemon=True).start()

    def _regen_worker(self, news, ch):
        try:
            self.log("🔄 배경 이미지 재생성 중... (다른 느낌으로 다시 뽑습니다)")
            prompt_img = build_image_prompt(news["title"], ch)
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            img_path = os.path.join(IMG_DIR, f"card_bg_{ts}.png")
            ok = False
            ...
            if ok:
                self.log(f"💾 저장: images/{os.path.basename(img_path)}")
                self.root.after(0, lambda: os.startfile(IMG_DIR))
            else:
                self.log("❌ 이미지 재생성 실패 — 잠시 후 다시 시도하세요")
        finally:
            self.busy = False
            self.root.after(0, lambda: self.btn_regen.config(state="normal"))

    # ---------------- 사용 기록 ----------------
    def show_history(self):
        used = self.history.get("used", [])
        win = tk.Toplevel(self.root)
        win.title(f"🗂 사용 기록 ({len(used)}건)")
        win.geometry("640x420")
        txt = tk.Text(win, font=("맑은 고딕", 9))
        txt.pack(fill="both", expand=True, padx=8, pady=8)
        if not used:
            txt.insert("end", "아직 사용한 뉴스가 없습니다.")
        for h in reversed(used[-100:]):
            txt.insert("end", f"[{h.get('date','')}] ({h.get('channel','')})\n  {h.get('title','')}\n\n")
